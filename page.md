...
- `je some_label` – A conditional jump that stands for "jump if equal." It only jumps if the last cmp found the respective values to be equal. Like `if (x == 5)` in a high-level language.
